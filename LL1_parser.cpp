...
private:
    ...
    void print_state(const string& action) {
        ...
        cout << setw(30)<< stackstr << setw(30) << inputstr << setw(30) << action << "\n";
    }
...
public:
    ...
    void parse() {
        cout << setw(30) << "Pila" << setw(30) << "Entrada" << setw(30) << "Acción" << endl;
        ...
        while (!stack.empty()) {
            ...
            if (isterminal(top)) {
                ...
                if (LL1_Parsing_Table[top].find(entradaActual) != LL1_Parsing_Table[top].end()) {
                    ...
                    string action = top + " -> ";
                    for (const auto& p : produccion) action += p + " ";
                    print_state(action);
                    stack.pop();
                    ...
                    cout << "Error de sintaxis: no se encontró entrada para " << tokenActual.lexema << endl;
                    return;
                }
            }
        }
